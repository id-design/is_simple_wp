<?php
/**
 * ISSimple_Theme_Options class.
 *
 * A custom builder settings page.
 * Based on the Odin_Theme_Options (@link https://github.com/wpbrasil/odin),
 * created by WPBrasil and with the licence GPLv2.
 *
 * @package		WordPress
...
class ISSimple_Theme_Options {
	...
	function scripts() {
		...
		if ( isset( $_GET['page'] ) && $this->id == $_GET['page'] ) {
			...
			wp_localize_script(
				...
				'issimpleAdminParams',
				array(
					'galleryTitle'  => __( 'Add images in gallery', 'issimple' ),
					'galleryButton' => __( 'Add in gallery', 'issimple' ),
					'galleryRemove' => __( 'Remove image', 'issimple' ),
					'uploadTitle'   => __( 'Choose a file', 'issimple' ),
					'uploadButton'  => __( 'Add file', 'issimple' ),
				)
			);
		}
	}
	...
	public function callback_color( $args ) {
		// Sets color class.
		$args['attributes']['class'] = 'issimple-color-field';
	...
	public function callback_upload( $args ) {
		...
		$html = sprintf( '<input type="text" id="%1$s" name="%2$s[%1$s]" value="%3$s" class="regular-text"%5$s /> <input class="button issimple-upload-button" id="%1$s-button" type="button" value="%4$s" />', $id, $tab, $current, __( 'Select file', 'issimple' ), $this->build_field_attributes( $attrs ) );
	...
	public function callback_image( $args ) {
		...
		$html  = '<div class="issimple-upload-image">';
	...
	public function callback_image_plupload( $args ) {
		...
		$html = '<div class="issimple-gallery-container">';
			$html .= '<ul class="issimple-gallery-images">';
				if ( ! empty( $current ) ) {
			...
			$html .= sprintf( '<input type="hidden" id="%1$s" name="%2$s[%1$s]" value="%3$s" class="issimple-gallery-field" />', $id, $tab, $current );

			// Adds "adds images in gallery" url.
			$html .= sprintf( '<p class="issimple-gallery-add hide-if-no-js"><a href="#">%s</a></p>', __( 'Add images in gallery', 'issimple' ) );
		$html .= '</div>';
	...
	public function validate_input( $input ) {
		...
		foreach ( $input as $key => $value ) {
			...
			if ( isset( $input[ $key ] ) ) {
				$output[ $key ] = apply_filters( 'issimple_theme_options_validate_' . $this->id, $value, $key );
			}
